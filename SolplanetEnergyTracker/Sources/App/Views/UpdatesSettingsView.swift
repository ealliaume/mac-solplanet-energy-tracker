...
/// Settings tab: current version + install type, the auto-check toggle, a manual
/// "Check now" with last-checked status, an inline install/restart control, and
/// the skip-version list.
struct UpdatesSettingsView: View {
    ...
    private var areUpdateChecksUnavailable: Bool {
        AppDelegate.areUpdateChecksUnavailable()
    }

    var body: some View {
        Form {
            if areUpdateChecksUnavailable {
                Section {
                    Text("Update checks are disabled for this build (running outside an installed .app).")
            ...
            Section("Automatic updates") {
                Toggle("Check for updates automatically", isOn: Binding(
                    get: { preferences.updatesAutoCheckEnabled },
                    set: { preferences.updatesAutoCheckEnabled = $0 }
                ))
                .disabled(areUpdateChecksUnavailable)
                Text("When enabled, the app checks GitHub for new releases at startup and every six hours.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            ...
            Section("Manual check") {
                HStack {
                    ...
                    .disabled(updateState.phase == .checking || areUpdateChecksUnavailable)
            ...
            if !preferences.updatesDismissedVersions.isEmpty {
                Section("Skipped versions") {
                    ForEach(preferences.updatesDismissedVersions, id: \.self) { version in
                        HStack {
                            Text(version).font(.system(size: 12))
                            Spacer()
                            Button(role: .destructive) {
                                preferences.updatesDismissedVersions.removeAll { $0 == version }
