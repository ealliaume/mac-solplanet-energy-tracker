...
/// progress lines into the UI. The app stays alive during the upgrade — `brew`
/// happily replaces the currently-running `.app` bundle on macOS (the running
/// mach-o is mmaped, so the on-disk swap doesn't fault the live process).
public actor BrewUpgradeRunner {
    ...
    public func runUpgrade(
        ...
        guard result.status == 0 else {
            await logger.log(.warning, "brew upgrade exit=\(result.status) lastLine=\(result.lastLine ?? "")")
    ...
    private func runStreaming(
        ...
        try await withCheckedThrowingContinuation { (cont: CheckedContinuation<StreamingResult, Error>) in
            // waitUntilExit() runs off the cooperative pool on a global queue
...
/// Buffers raw bytes coming off two pipes, splits on newlines, and forwards each
/// non-empty line. The trailing partial chunk (no final newline) is emitted by
/// `flushTrailing()` after the process exits.
