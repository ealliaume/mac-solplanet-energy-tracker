...
/// In-popover banner offering Install / Restart and streaming install progress,
/// driven by `UpdateState.Phase`. Replaces the old "view release" link.
struct UpdateAvailableBanner: View {
    ...
    private var subtitle: String {
        switch installationKind {
        case .homebrewCask:
            return "A new version is available. It will be reinstalled via Homebrew."
        case .manual, .none:
        ...
        .sheet(isPresented: $showingReleaseNotes) {
            if let notes = update.releaseNotes, !notes.isEmpty {
                ReleaseNotesSheet(
                    ...
                    onDismiss: { showingReleaseNotes = false }
                )
            }
        }
    }
    ...
    private var idleSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if hasReleaseNotes {
                ...
                .buttonStyle(.link)
                .help("View release notes for this version")
            }
    ...
    private var hasReleaseNotes: Bool {
        guard let notes = update.releaseNotes else { return false }
