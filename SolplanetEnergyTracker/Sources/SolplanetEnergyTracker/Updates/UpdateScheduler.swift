...
/// Periodically polls `UpdateChecker` and updates an `UpdateState`. Runs an
/// initial check shortly after start, then on a fixed interval, and de-dupes the
/// proactive notification so the banner/alert doesn't re-pop every poll.
public actor UpdateScheduler {
    ...
    /// Last version the scheduler proactively notified about — used to avoid
    /// re-popping the alert on every poll while the version is unchanged.
    ...
    public func checkOnce(forceUserVisible: Bool = false) async {
        ...
        do {
            ...
            await logger.log(.warning, "UpdateScheduler: check failed: \(error)")
