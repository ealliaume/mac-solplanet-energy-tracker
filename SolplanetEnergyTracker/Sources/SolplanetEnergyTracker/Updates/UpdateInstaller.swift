...
/// Strategy describing how to finalize a previously-prepared update — i.e. swap
/// the staged bundle into place (manual installs) or simply relaunch (Homebrew
/// installs, where `brew upgrade --cask` already replaced the bundle).
///
...
public struct UpdateFinalizationPlan: Sendable, Equatable {
    ...
    /// process. The script waits for parent exit, performs the final swap
    /// (manual only), and relaunches the app.
...
public actor UpdateInstaller {
    ...
    /// Seconds the finalize script polls for the parent to exit (60 × 0.5 s).
    private static let parentExitPollAttempts = 60
    private static let scriptPermissions: Int = 0o755
    ...
    private static func defaultScriptDirectory() -> URL {
        ...
        return home.appendingPathComponent(".cache/\(AppInfo.cacheDirectoryName)/updates", isDirectory: true)
    }

    /// Standard staging directory for downloaded / extracted bundles — exposed
    /// so the downloader and installer agree on the location.
    ...
    public func buildManualFinalizationPlan(
    ...
    ) async throws -> UpdateFinalizationPlan {
        ...
        await logger.log(.info, "Built manual finalize plan: \(summary) → \(scriptURL.path) (admin=\(requiresAdmin))")
    ...
    /// Build a finalization plan for the Homebrew path: brew has already replaced
    /// the bundle, so we only need to wait for parent exit and relaunch as the
    /// console user.
    public func buildHomebrewFinalizationPlan(
        ...
        await logger.log(.info, "Built brew finalize plan: \(summary) → \(scriptURL.path)")
        return UpdateFinalizationPlan(
            scriptPath: scriptURL.path,
            summary: summary,
            requiresAdminPrivileges: false // brew never needs admin for relaunch
        )
    }
    ...
    private static func writeScript(_ script: String, to url: URL, fileManager: FileManager) throws {
        ...
        try fileManager.setAttributes([.posixPermissions: scriptPermissions], ofItemAtPath: url.path)
    }
    ...
    /// writable. Missing bundle paths fall back to false (treat as "needs admin"
    /// rather than silently succeed-then-fail at runtime).
    ...
    private static func manualSwapScript(
        ...
        for i in $(seq 1 \(parentExitPollAttempts)); do
    ...
    /// Relaunch as the console user even when the script runs as root — when the
    /// script is invoked through `osascript ... with administrator privileges`,
    /// `/usr/bin/open` would otherwise launch the app as root and poison
    /// subsequent file ownership.
