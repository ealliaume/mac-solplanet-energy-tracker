...
/// downloaded, then a single `verifying` and `extracting` notification.
...
public actor UpdateDownloader {
    ...
    private static let hashChunkSizeBytes = 1 * 1024 * 1024 // 1 MiB
    private static let extractTimeoutSeconds = 120
    ...
    public func downloadAndStage(
        ...
        if let shaURL = update.sha256URL {
            onEvent(.verifying)
            try await verifySHA256(file: zipURL, expectedFrom: shaURL)
        }
        ...
        await logger.log(.info, "Update staged at \(stagedApp.path)")
        return stagedApp
    }
    ...
    private func downloadFile(
        ...
        // The observer already moved the system temp file to a stable path;
        // here we just rename it into the destination atomically.
    ...
    private func verifySHA256(file: URL, expectedFrom shaURL: URL) async throws {
        ...
        guard expected.allSatisfy({ $0.isHexDigit }), expected.count == Self.sha256HexLength else {
            throw UpdateDownloaderError.shaParseFailed(raw: raw)
        }
    ...
    private func sha256Hex(of fileURL: URL) throws -> String {
        ...
        var hasher = SHA256()
        // Chunked so the resident set stays bounded on large zips.
        while true {
            let chunk = handle.readData(ofLength: Self.hashChunkSizeBytes)
    ...
    private func extractZip(zipURL: URL, into destDir: URL) async throws {
        ...
        do {
            result = try await process.run(
                ...
                timeoutSeconds: Self.extractTimeoutSeconds
            )
        } catch {
    ...
    private func findAppBundle(in directory: URL) throws -> URL? {
        // The zip layout is `<archive root>/Solplanet Battery Energy Tracker.app`,
        // but tolerate one nested wrapper directory in case the build pipeline
        // ever changes.
...
/// Bridges URLSession's delegate-based download progress callbacks to async/await
/// while keeping the staged temp file alive past the delegate's
/// `didFinishDownloadingTo` call (the system deletes the original shortly after
...
private final class DownloadObserver: NSObject, URLSessionDownloadDelegate, @unchecked Sendable {
    ...
    func urlSession(
        ...
        let stable = FileManager.default.temporaryDirectory
            .appendingPathComponent("solplanet-tracker-dl-\(UUID().uuidString).zip")
        do {
