...
public final class UpdateState {
    public enum Phase: Sendable, Equatable {
        case idle
        case checking
        /// Right after the user clicked "Install": building plan, resolving brew.
        case preparing
        ...
        case extracting
        /// Homebrew install: `brew upgrade --cask` running, with the last log
        /// line from stdout/stderr (truncated) for user feedback.
        case runningHomebrew(lastLine: String?)
        /// All work done; the user can click "Restart" whenever ready. For manual
        /// installs the new bundle is staged at `stagedAppPath` and swapped in by
        /// the finalize script. For Homebrew the swap is already complete —
        /// restart simply relaunches the bundle.
        case readyToRestart
        /// User clicked "Restart": finalize script launched, app is about to quit.
        case restarting
        /// Anything failed during install. Banner shows the message with Retry.
        case failed(message: String)
    }

    public private(set) var phase: Phase = .idle
    public private(set) var availableUpdate: AvailableUpdate?
    public private(set) var latestKnownVersion: SemanticVersion?
    public private(set) var lastCheckedAt: Date?
    public private(set) var lastError: String?
    public private(set) var installationKind: InstallationKind?
    /// Set during the manual install flow once extraction succeeds — the path to
    /// the staged `.app` bundle ready to swap into place.
    ...
    public func setAvailable(_ update: AvailableUpdate?, latestVersion: SemanticVersion?, kind: InstallationKind?, checkedAt: Date) {
